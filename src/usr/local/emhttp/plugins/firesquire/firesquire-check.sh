...
#   firesquire-check.sh --json     machine-readable JSON (consumed by the WebGUI)
#
...
if have smartctl; then
  ...
  for d in $devs; do
    [ -z "$d" ] && continue
    _ssidx=$((_ssidx+1))
    [ "$PROGRESS" -eq 1 ] && [ "${_sstot:-0}" -gt 0 ] && fspush $((72 + _ssidx*24/_sstot)) p_smart
...
if [ -f "$_ssdir/banner.txt" ]; then
  ...
  echo "   FireSquire  -  advisory pre-reboot health check for your Unraid box"
fi

echo
banner "  ===== FIRESQUIRE: $VERDICT ====="
echo
